...
using System.Text;
StringBuilder dynam = new StringBuilder();
...
/* если захотите проверить работоспособность кода
massiv1[0, 2] = 3;
massiv1[0, 1] = 2;
massiv1[0, 0] = 1;
massiv2[0, 0] = 3;
...
void Zerkalo(int[,] massiv1, int[,] massiv2, StringBuilder dynam) //принимает в аргументы обе матрицы и стрингбилдер
{
    ...
    {
        for(int k = 0; k < massiv2.GetLength(0); k++)
        {
            ...
            {
                if (massiv1[i, j] == massiv2[k, eleme]) //если значение ячейки из матрицы 1 совпало с зеркальным симметричным значением матрицы 2, то мы увеличиваем popad на 1
            ...
            if (popad == massiv1.GetLength(1)) //если колчество зеркально симметричных элеменнтов равно количеству элементтов в строке, то добавляем номера строк в стринглбилдер
            {
                dynam.AppendLine($" первая матрица {i + 1} - вторая матрица {k+1}"); //добавляю i+1 и k+1 чтобы пользователю было понятнее о какой строке идёт речь
            }
        }
        
    }
}
void Vivod(int[,] massiv1, int[,] massiv2,StringBuilder dynam)// принимает в аргументы оби матрицы и стрингбилдер
{
    ...
    if (dynam.Length == 0) //если в матрицах нет зеркально симметричных строк, то выводим что их нет
    {
